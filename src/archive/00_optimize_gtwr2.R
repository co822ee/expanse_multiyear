...
for(yr_i in seq_along(csv_names)){
   ...
   foreach(param_i = 1:nrow(gtwr_param))  %dopar%  {
      ...
      nfold_validation <-  lapply(1:nfold, function(fold_i){
         ...
         # Output the predictions at validation points (the validation subset from the training data)
         calibr_GTWR2(param_i, nfold, yrs_v=years[[yr_i]], grd=grd2, train_df=train_sub, param=gtwr_param)
      }) %>% do.call(rbind, .)
