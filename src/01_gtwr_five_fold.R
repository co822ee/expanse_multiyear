...
for(yr_i in seq_along(csv_names)){
   ...
   foreach(fold_i = 1:nfold)  %dopar%  {
      ...
      if(!file.exists(paste0('data/workingData/GTWR_result_all_', csv_name_fold, '.csv'))){
         ...
         gtwr_valid2 <- gtwr_valid %>% dplyr::select(!pred_c, 'year', 
                                                     -c('system.index', '.geo', 'ycoord','xcoord'))
         write.csv(gtwr_valid2 , paste0('data/workingData/GTWR_result_all_', csv_name_fold, '.csv'), row.names = F)
      }
   }
   stopCluster(cl)
   gc()
