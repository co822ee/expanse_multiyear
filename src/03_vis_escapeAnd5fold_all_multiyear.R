...
dat_r2 <- dat_r2[!is.na(dat_r2$values), ] %>% 
   mutate(model2=ifelse(period%in%c('08-10', '09-11', '10-12', 
                                   '08-12', '06-12', '12-19', '00-19'), paste0(model, period), model))
...
## 
dat_r2_all <- rbind(dat_r2, dat_r2_escape)
dat_r2_all <- dat_r2_all %>%
   # filter(model!='rf'&model!='rf00-19') %>% 
   mutate(model2=factor(model2, levels = unique(dat_r2_all$model2)[rev(mixedorder(unique(dat_r2_all$model2)))]),
          model = factor(model, levels=c('rf', 'gtwr', 'slr')),
          data=factor(data, levels = unique(dat_r2_all$data)))

ggplot(dat_r2_all, 
       aes(x=reorder(model2, as.numeric(model)), y=values, 
...
ggsave('graph/R2_v2_multiyear.tiff', width=7.5, height=10, units='in', dpi=200)

########## All algorithms (single-year and 00-19)######
...
## diff
dat_r2_diff1 <- em_df_all %>% filter(EM=='rsq', !period%in%c('08-10', '09-11', '10-12', 
                                                       '08-12', '06-12', '12-19')) %>% 
   filter(period!='00-19') %>% 
   dplyr::select(-gtwr) %>% 
   mutate(slr=(slr-gwr)*100, rf=(rf-gwr)*100)
dat_r2_diff2 <- em_df_all %>% filter(EM=='rsq', !period%in%c('08-10', '09-11', '10-12', 
                                                             '08-12', '06-12', '12-19')) %>% 
   filter(period=='00-19') %>% 
   dplyr::select(-gwr) %>% 
   mutate(slr=(slr-gtwr)*100, rf=(rf-gtwr)*100)

ggplot(dat_r2_diff2 %>% gather("model", "values", -c("year", "EM", 'poll', 'period')),
       aes(x=year, y=values))+
   geom_point()+
   labs(title='multiyear')+
   facet_grid(poll~model)

ggplot(dat_r2_diff1 %>% gather("model", "values", -c("year", "EM", 'poll', 'period')),
       aes(x=year, y=values))+
   geom_point()+
   labs(title='singleyear')+
   facet_grid(poll~model)
