...
run_GTWR <- function(fold2_i, grid_i, yrs_v){
   ...
   gtwr_valid <- lapply(yrs_v, trainGTWR, sp_train1=sp_train2, grid_i=grid_i,
                        ...
                        target_poll=target_poll, eq=eq) %>% do.call(rbind, .)  ## Should be the same size as validation data
   gtwr_valid
}

## Train GTWR for each year
trainGTWR <- function(sp_train1, gtwr_yr, grid_i, sp_valid1, valid_sub1, target_poll, eq){
