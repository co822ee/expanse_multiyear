...
validation_str <- c('escape', 'random')
...
csv_names <- lapply(validation_str, function(valid_str){
   if(valid_str=='escape'){
      target_pollv = c('NO2', 'PM2.5', 'PM10')
   }else{
      target_pollv = c('NO2', 'PM2.5', 'PM10', 'O3')
   }
   lapply(target_pollv, function(target_poll){
      paste0(valid_str, '_', target_poll, "_", c('08-10', '09-11', '10-12', 
                                                 '08-12', '06-12', '12-19', '00-19'))  
   }) %>% unlist
}) %>% unlist

years <- rep(list(2008:2010, 2009:2011, 2010:2012, 
                  2008:2012, 2006:2012, 2012:2019, 2000:2019), length(csv_names)/7)
